...
class TestRecorder(object):
    ...
    def _register_move_with_description(self, timestep, entity_name, end_position, description="Move"):
        self._current_position[entity_name] = end_position
    ...
    def register_move_left(self, timestep, entity_name):
        ...
        end_position = Node(coordinates=(current_position.get_x() - 1, current_position.get_y()))
        self._register_move_with_description(entity_name, timestep, end_position, description="MoveLeft")

    def register_move_right(self, timestep, entity_name):
        current_position = self._current_position[entity_name]
        end_position = Node(coordinates=(current_position.get_x() + 1, current_position.get_y()))
        self._register_move_with_description(entity_name, timestep, end_position, description="MoveRight")

    def register_move_up(self, timestep, entity_name):
        current_position = self._current_position[entity_name]
        end_position = Node(coordinates=(current_position.get_x(), current_position.get_y() + 1))
        self._register_move_with_description(entity_name, timestep, end_position, description="MoveUp")

    def register_move_down(self, timestep, entity_name):
        current_position = self._current_position[entity_name]
        end_position = Node(coordinates=(current_position.get_x(), current_position.get_y() - 1))
        self._register_move_with_description(entity_name, timestep, end_position, description="MoveDown")

    def register_wait(self, timestep, entity_name):
        register_action = self._action_list.append(WaitAction(timestep, entity_name))

    def register_appearance(self, entity_name, timestep, start_position):
        self._current_position.update({entity_name: start_position})
        register_action = self._action_list.append(AppearAction(timestep, entity_name, start_position))

    def register_disappearance(self, entity_name, timestep):
        self._current_position.update({entity_name: None})
        register_action = self._action_list.append(DisappearAction(timestep, entity_name))
